...
{
    bool followPV, scorePV, color;
    int plyOut;
    int[] maxAttack = { 1, 2, 8, 16, 16, 32, 8 }, probabilities, pieceValues = { 0, 1, 3, 3, 5, 9, 100 }, vicScore = { 0, 100, 200, 300, 400, 500, 600 }, pvLength = new int[64], mateFiles = {-3,-2,-1,0,0,-1,-2,-3};
    ...
    {
        float evaluation = 0;
        float endgameweight = (10 - BitboardHelper.GetNumberOfSetBits(board.AllPiecesBitboard)) * 0.1f;
        ...
        for (int i = 0; i < 8;++i)
        {
            for (int j = 0; j < 8; ++j)
            {
                ...
                evaluation += (pieceValues[(int)piece.PieceType] + (piece.IsKing && color == piece.IsWhite ? endgameweight : 1) * activity * 0.1f / (maxAttack[(int)piece.PieceType])) * (piece.IsWhite ? 1 : -1);
            } 
        } 
